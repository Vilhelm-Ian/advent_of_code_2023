...
fn generate_loop_part_2(input: &str) -> Vec<[usize; 2]> {
    ...
    let mut result = vec![current_node.position];
    while current_node.position != starting_point {
        ...
        current_node = hash_map
            .get(&next_node_location)
            .expect("node dosen't exist in hashmap");
        result.push(current_node.position);
    }
    result
}

fn is_connected_with_
...
L---J";
    ...
    #[test]
    fn part_2_first_test() {
        let input = generate_loop_part_2(input);
        assert_eq!(14, result);
    }
}
